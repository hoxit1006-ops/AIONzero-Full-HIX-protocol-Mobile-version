...
This script auto-detects your computer's LAN IPv4 and forces Expo to advertise that address (instead of `127.0.0.1`).
...
5. Confirm Expo is not advertising localhost:
   - In terminal, the URL should look like `exp://192.168.x.x:8081` (or `10.x.x.x`).
   - If you still see `127.0.0.1`, restart with `npm run start:lan` and make sure VPN adapters are disabled.
